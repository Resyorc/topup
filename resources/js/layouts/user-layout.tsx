...
function NavLinks({ onNav, url, avatarUrl, initials, user }: NavLinksProps) {
    return (
        <>
            {/* User Card */}
            <div className="mb-2 rounded-2xl border border-border-light bg-card p-4">
                <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-white/10 text-sm font-bold text-white ring-2 ring-primary/40">
...
<Link href="/dashboard/coin-history" onClick={onNav} className={`flex items-center gap-3 rounded-xl px-6 py-4 transition ${url.includes('/dashboard/coin-history') ? 'bg-gradient-to-r from-primary to-[var(--color-primary-light)] font-semibold text-white shadow-lg shadow-primary/20' : 'font-medium text-gray-300 hover:bg-white/5 hover:text-white'}`}>
            ...
            <Link href="/dashboard/settings" onClick={onNav} className={`flex items-center gap-3 rounded-xl px-6 py-4 transition ${url.includes('/dashboard/settings') ? 'bg-gradient-to-r from-primary to-[var(--color-primary-light)] font-semibold text-white shadow-lg shadow-primary/20' : 'font-medium text-gray-300 hover:bg-white/5 hover:text-white'}`}>
            ...
            <div className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 gap-8 md:grid-cols-12">
                    ...
                    <aside className="hidden flex-col gap-2 md:col-span-3 md:col-start-1 md:flex">
                        <NavLinks url={url} avatarUrl={avatarUrl} initials={initials} user={user} />
                    </aside>
