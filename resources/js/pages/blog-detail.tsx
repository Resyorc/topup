...
function ShareButtons({ title, slug }: { title: string; slug: string }) {
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <button
                onClick={copyLink}
                className="flex items-center gap-1.5 rounded-lg border border-(--color-border-light) bg-(--color-bg-card) px-3 py-1.5 text-xs font-medium text-gray-300 transition hover:border-primary/40 hover:text-white"
            >
                {copied ? (
                    <>
            ...
            <a
                ...
                className="flex items-center gap-1.5 rounded-lg border border-(--color-border-light) bg-(--color-bg-card) px-3 py-1.5 text-xs font-medium text-gray-300 transition hover:border-green-500/40 hover:text-green-400"
            >
...
function TableOfContents({ toc, activeId }: { toc: TocItem[]; activeId: string }) {
    ...
    return (
        <nav className="rounded-xl border border-(--color-border-light) bg-(--color-bg-card) p-4">
            <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-gray-500">Daftar Isi</p>
            <ul className="space-y-0.5 border-l-2 border-(--color-border-light) pl-0">
                {toc.map((item) => (
                    <li
                        key={item.id}
                        className={`border-l-2 -ml-0.5 transition-colors ${activeId === item.id ? 'border-primary' : 'border-transparent'}`}
...
function RelatedCard({ article }: { article: RelatedArticle }) {
    return (
        <Link
            ...
            className="group flex gap-4 rounded-xl border border-(--color-border-light) bg-(--color-bg-card) p-4 transition hover:border-primary/40"
        >
            <div className="h-20 w-28 shrink-0 overflow-hidden rounded-lg bg-(--color-bg-main)">
                {article.thumbnail ? (
                    <img src={article.thumbnail} alt={article.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
                ) : (
            ...
            <div className="min-w-0 flex-1">
                ...
                <h4 className="mt-1 line-clamp-2 text-sm font-bold leading-snug text-white transition group-hover:text-primary">
                    {article.title}
                </h4>
            </div>
        </Link>
...
export default function BlogDetail({ article, related_articles }: BlogDetailProps) {
                ...
                <div className="flex gap-10">
                    ...
                    <article ref={articleRef} className="min-w-0 flex-1">
                        ...
                        <header className="mb-10">
                            ...
                            {/* Meta row */}
                            <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-(--color-border-light) pt-5">
